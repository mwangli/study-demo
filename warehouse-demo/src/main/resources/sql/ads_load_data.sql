...
from (
         ...
         from (
                  ...
                  where dt >= date_add('2022-05-19', -recent_days + 1)
              ) t
                 ...
                 (
                     ...
                     where id not in
                           (
                               select user_id
                               from dwt_user_action_7daycount
                               where dt = date_add('2022-05-19', -1)
                                 and login_count > 0
                           )
                 )
       ),
...
select t1.date_id,
       ...
       date('2022-05-19')
from (
         select '2022-05-19'                                                   date_id,
                date_format(create_time, 'yyyy-MM-dd')                         create_date,
                datediff('2022-05-19', date_format(create_time, 'yyyy-MM-dd')) retention_days,
                count(1)                                                       new_user_count
         from ods_user_info
         where create_time >= date_add('2022-05-19', -7)
           and create_time < '2022-05-19'
